      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">
             Role Anda: <span className={cn("font-bold uppercase", userRole === "SUPER_ADMIN" ? "text-red-500" : "text-blue-500")}>{userRole.replace('_', ' ')}</span>
          </p>
        </div>
        <Link href="/admin/work/create" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm transition-colors shadow-lg shadow-blue-900/20">
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-xl overflow-visible shadow-xl">
        {loading ? (
            <div className="p-12 text-center flex flex-col items-center gap-3 text-gray-500">
                ...
                <span className="text-sm">Memuat karya mahasiswa...</span>
            </div>
        ) : data.length === 0 ? (
            <div className="p-12 text-center text-gray-500">
                Belum ada project / karya yang diupload.
            </div>
        ) : (
          <table className="w-full text-left border-collapse">
            <thead className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              <tr>
                ...
                <th className="p-4 border-b border-white/10">Judul Project</th>
                <th className="p-4 border-b border-white/10">Mahasiswa</th>
              ...
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="p-4 font-medium text-white flex items-center gap-3">
                    <div className="w-8 h-8 rounded bg-blue-600/20 text-blue-500 flex items-center justify-center border border-blue-500/20 shrink-0">
                        <Code2 size={16} />
                    </div>
                    {item.title}
                  </td>
